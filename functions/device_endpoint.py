...
from functions.format_list import (
    format_device_list,
    format_lcd_independence_list,
    format_lcd_list,
)
from functions.get_selection import (
    get_device_selection,
    get_lcd_independence_selection,
    get_lcd_selection,
)
...
def get_device_info():
    try:
        response = requests.get("https://app.divoom-gz.com/Device/ReturnSameLANDevice")
        ...
        return device_list[selected_device].get("DeviceId"), device_list[
            selected_device
        ].get("DevicePrivateIP")
...
def get_lcd_info(device_id):
    try:
        response = requests.get(
            f"https://app.divoom-gz.com/Channel/Get5LcdInfoV2?DeviceType=LCD&DeviceId={device_id}"
        )
        data = response.json()
        ...
        if not lcd_independence_list:
            print(
                "LcdIndependenceList is empty. Assuming device is Pixoo64 or similar."
            )
            print(
                "LCD Independence set to 0, LCD Index set to 0, LCD Clock ID set to 625"
            )
            return 0, 0, 625
        ...
        if len(lcd_independence_list) > 1:
            ...
            selected_lcd_independence = get_lcd_independence_selection(
                lcd_independence_list
            )
        else:
